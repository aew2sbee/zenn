...
:::message alert
本記事は、書籍と同じ **Next.js 14.2** 時点の create-next-app の画面です。
現在の create-next-app（`npx create-next-app@latest`）では、質問の数・順序・文言が変わっています（例: ESLint の質問が linter の選択に変更、Turbopack の質問を追加）。
同じ画面で進めたい場合は、`npx create-next-app@14` を実行してください。
:::
...
# デフォルトのimport alias をカスタマイズしますか？
...
A. 例えば、`next-js`というプロジェクトを作成したら、下記のようなディレクトリ構成になります（主要なものだけ抜粋。`.next`は`npm run dev`などの実行後に生成されます）。
...
TypeScript は、静的型チェックによってバグを防ぐ役割を果たすため、`Yes`と回答します。
...
- コード品質を保つ：`<img>`要素に`alt`属性を設定し忘れるなど、アクセシビリティの観点で望ましくないコードを検知します。
- 潜在的なバグの混入を防ぐ：ライブラリ API を使用している箇所で非推奨な書き方をしているコードを検知します。
- 開発者同士のレビュー指摘を減らす：`import/order`などのルールを追加すれば、import 文の並びといったコーディングガイドラインにまつわる細かい指摘が不要になります（既定の設定には含まれていません）。

---

#### Would you like to use Tailwind CSS? … No / Yes - Tailwind CSS を使いますか？

Tailwind CSS は、CSS のフレームワークです。目的のプロジェクトに合わせて回答してください。
...
ルーティング機能として、従来の機能（Pages Router）と新しい機能（App Router）のどちらを使うかを尋ねています。新規プロジェクトでは App Router が推奨されているため、`Yes`と回答します。

---

#### Would you like to customize the default import alias (@/\*)? … No / Yes - デフォルトの import alias をカスタマイズしますか？

こだわりがない場合は、`No`と回答します。
`No`と回答すると、既定の`@/*`（`src/`を使う場合は`./src/*`を指す）がそのまま設定され、下記のように import 文のパスを短く書けます。
