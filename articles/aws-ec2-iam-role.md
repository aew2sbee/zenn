...
topics: ["aws", "iam", "mfa"] # タグ。["markdown", "rust", "aws"]のように指定する
...
`ReadOnlyAccess`のユーザーを作成後にMFAの認証を行うとしましたが、下記のエラーメッセージが表示されました。
...
4. 登録するデバイス名を`デバイス名`の入力欄に記入する
    ...
    **NG** : Aさん=iPhone, Bさん=iPhone
    **OK** : Aさん=iPhone-A, Bさん=iPhone-B
    :::
    :::message alert
    【注意２】
