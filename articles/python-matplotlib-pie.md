...
そこで、matplotlib で円グラフを描く方法を学びました。
...
# counterclock=False: 時計回りに並べる（既定は反時計回り）
# labeldistance=0.5: ラベル（indexの値）を円の内側に表示する（既定は1.1で外側）
df["つぶあん"].plot.pie(startangle=90, counterclock=False, labeldistance=0.5)
plt.show()
```

:::message
- 左側の「つぶあん」は、pandas が Series の名前を y 軸ラベルとして自動で設定したものです。消す場合は`plt.ylabel("")`を使います。
