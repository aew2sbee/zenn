...
参考書籍: 『探索的テストの考え方』（James A. Whittaker 著）
...
基礎的な知識とは、計算機科学、アルゴリズム、計算複雑性理論、グラフ理論、データ構造、テスト知識などです。

また、テスト設計ができても**観点の抜け漏れが多いテストシナリオ**になる可能性があります。
なので、まずは**基礎的な「コンピューターサイエンス」などの学習**を行うことをすすめます。

## 🌱 1. 高品質なサービスにする

つぎに、目の前のアサインされているサービスの不具合を検出し PJ を**成功させる**ことが最優先です!
...
たとえば、下記の記事のようにテスト専用の環境を作成し、即座に誰でもテストを実行できるようにするとよいですね。
...
現状に満足せず、**今よりも「効率よく」「効果的に」「迅速に」不具合**を検出することを常に意識し、最新のライブラリやツールの情報収集や学習、検討を行います。
また、自分の考え方に**「誤り」や「勘違い」がないのか？**を自問自答します。

## 🌱 5. 教育

これまでの取り組みを十二分に達成できたら、次は自分と同じレベルのテスト技術者を育成する番です。
