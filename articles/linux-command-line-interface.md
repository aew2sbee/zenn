...
:::message
実行結果は、執筆時点の `ubuntu:latest`（Ubuntu 22.04）のものです。現在の `ubuntu:latest` は別のバージョンを指すため、結果が異なる場合があります。同じ結果を確認したい場合は、`ubuntu` の代わりに `ubuntu:22.04` を指定してください。
:::
...
docker コマンドを使って Linux(Ubuntu)環境を用意します。
...
version 2.34.1 がインストールされていることが確認できました。
...
`ll` は、Ubuntu の `.bashrc` に定義されている `ls -alF` の別名（エイリアス）です。`ll` が使えない環境では `ls -alF` を実行してください。
...
/root（root ユーザーのホームディレクトリ ~）配下に sample という名前で作成します。
...
ホームディレクトリ（~）に戻り、.bashrc の中身を表示します。
...
sample/ に移動し、sample/ 配下にある memo.md を1つ上のディレクトリ（~）に移動します。
