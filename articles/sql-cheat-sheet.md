...
topics: ["sql", "postgresql", "初心者向け"] # タグ。["markdown", "rust", "aws"]のように指定する
...
:::message alert
`DELETE`や`UPDATE`で`WHERE`を省略すると、テーブルの全ての行が対象になります。実行する前に、同じ条件の`SELECT`で対象の行を確認しましょう。
:::
...
```sql:基本構文
INSERT INTO テーブル名 (カラム名A, カラム名B, カラム名C) VALUES
...
```sql:サンプル
-- sales に 8 件のデータを作成する（id は SERIAL により 1 から自動で採番される）
INSERT INTO sales (date, item_name, item_cont, price, customer, discount, discount_rate) VALUES
    ('2024-01-01', 'リンゴ', 3, 260, 'Aさん', TRUE, 0.1),
    ('2024-01-02', 'バナナ', 2, 340, 'Bさん', FALSE, NULL),
    ('2024-01-03', 'イチゴ', 4, 550, 'Cさん', FALSE, NULL),
    ('2024-01-04', 'リンゴ', 5, 670, 'Dさん', FALSE, NULL),
    ('2024-01-05', 'バナナ', 1, 560, 'Eさん', FALSE, NULL),
    ('2024-01-06', 'バナナ', 4, 340, 'Fさん', TRUE, 0.5),
    ('2024-01-07', 'イチゴ', 6, 230, 'Gさん', FALSE, NULL),
    ('2024-01-08', 'リンゴ', 2, 410, 'Hさん', TRUE, 0.3);
```
...
```sql:サンプル
-- sales と items で id が一致する行の sales.item_name を取得する
SELECT sales.item_name FROM sales
...
```sql:サンプル
-- sales と items で id が一致する行のうち、item_cont が 2 より多い行の sales.item_name を取得する
SELECT sales.item_name FROM sales
INNER JOIN items ON sales.id = items.id
WHERE sales.item_cont > 2
```
...
```sql:基本構文
SELECT * FROM テーブル名 WHERE カラム名 BETWEEN 値A AND 値B
```

```sql:サンプル
-- price が 300 以上 400 以下（両端を含む）に該当する値を取得する
...
```sql:サンプル
-- item_name ごとのデータ数が 3 件以上のグループを取得する
...
```sql:基本構文
SELECT カラム名A, 集計関数 FROM テーブル名 GROUP BY カラム名A
```
...
### 15. 特定の範囲を満たさないデータを取得する

```sql:基本構文
SELECT * FROM テーブル名 WHERE カラム名 NOT BETWEEN 値A AND 値B
```

```sql:サンプル
-- price が 300 以上 400 以下（両端を含む）に該当しない値を取得する
...
:::message
`NOT IN`のリストに`NULL`が含まれると、1件も取得できません。
:::
...
```sql:サンプル
-- item_name = `リンゴ`か、price が 500 以上に該当する値を取得する
SELECT * FROM sales WHERE item_name = 'リンゴ' OR price >= 500
```
...
| \*     | 掛け算               |
| /      | 割り算               |
| %      | 割り算の余りを求める（Oracle では`MOD()`関数を使う） |

### SQL の論理的な処理順序

`SELECT`文の各句は、下記の順序で処理されると考えるとわかりやすいです。
実際の実行計画は、DBMSのオプティマイザが決めるため、この順序と異なる場合があります。

| 順序 | 説明                               | 句                   |
| ---- | ---------------------------------- | -------------------- |
| 1    | データを取り出すテーブルを指定する | FROM 句              |
| 2    | テーブルを結合する                 | INNER JOIN 句, ON 句 |
| 3    | 取り出すデータの条件を付ける       | WHERE 句             |
| 4    | グループ化する                     | GROUP BY 句          |
| 5    | グループ化した後に条件を付ける     | HAVING 句            |
| 6    | 取り出すカラムを指定する           | SELECT 句            |
| 7    | 重複を取り除く                     | DISTINCT             |
| 8    | レコードを並べ替える               | ORDER BY 句          |
| 9    | 取得する件数を制限する             | LIMIT 句             |
