---
title: "[Next.js] Root layout ファイルでheaderタグとfooterタグを共通化する" # 記事のタイトル
...
下記書籍の「Root layout ファイル」について学びがあったので、記事として記録します。
...
:::message
`create-next-app` で App Router・`src` ディレクトリ・Tailwind CSS を有効にして作成した、Next.js 14（書籍準拠）のプロジェクトを前提とします。
Next.js 15 以降では、テンプレートのフォントが Inter ではなく Geist になっています。
:::
...
:::message alert
※ header タグと footer タグが分かりやすいように**背景色**を付けています。
:::
  ...
  return (
    <html lang="ja">
      <body className={inter.className}>
        ...
        {/* children に各ページ（page.tsx）の内容が入る */}
        <main>{children}</main>
...
`http://localhost:3000/`にアクセスした時に表示される内容を、下記の通りにコーディングします。
...
起動後、`http://localhost:3000/` にアクセスして確認します。

:::message
header タグ、footer タグ、main タグの内容が表示されている
:::

![next-js-layout-page](/images/articles/nextjs-root-layout-file/next-js-layout-page.png)

`http://localhost:3000/sample` にアクセスして確認します。

:::message
header タグと footer タグが共通化されている
main タグ内には別の内容が表示されている
:::
